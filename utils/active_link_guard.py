# -*- coding: utf-8 -*-
"""Active-link order protection — one active order per normalized target link.

``orders`` remains the single source of truth. Occupancy is global across
users/services/providers and is keyed only by the normalized link.
"""
...
# Status keys after normalize_order_status_key (underscore → space, lowercased).
# pending_admin is included: Telegram already creates a real unresolved order
# (often with provider submission) that occupies the customer target link.
...
# Exact status strings as commonly stored (for SQL partial-index WHERE).
...
def normalize_order_link(link: object) -> str:
    ...
    if not (lowered.startswith("http://") or lowered.startswith("https://")):
        # Multi-line or opaque targets: occupancy key is the full trimmed payload.
        return raw
    ...
    # urlsplit may leave userinfo; ignore credentials in occupancy key.
...
def active_link_status_sql_predicate(column: str = "status") -> str:
    """SQL fragment: true when *column* is an active link-occupying status."""
    return f"LOWER(REPLACE({column}, '_', ' ')) IN ('pending', 'pending admin', 'submitted', 'in progress', 'processing')"
...
def find_active_order_for_link(
    ...
    """Return the oldest active order occupying *link*, or None.

    Uses ``normalized_link`` when set; also scans active rows whose
    ``normalized_link`` is empty (pre-migration / conflict leftovers) by
    normalizing the stored ``link`` in Python.
    """
    ...
    # Legacy / conflict leftovers without normalized_link.
